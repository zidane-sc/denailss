...
const UPCOMING_STATUSES: readonly string[] = ["confirmed", "pending_deposit", "waiting_verification"];

export default function BookingsPage() {
  const [activeTab, setActiveTab] = useState<TabState>("upcoming");

  const filteredBookings = CUSTOMER_BOOKINGS.filter((b) => {
    if (activeTab === "cancelled") return b.status === "cancelled";
    if (activeTab === "past") return b.status === "completed" || b.status === "no_show";
    // upcoming: any non-terminal, non-cancelled status
            ...
            <RevealItem key={booking.id}>
              <div className="flex flex-col gap-4 rounded-2xl border border-border bg-card p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-start gap-4">
                  <div className="mt-1 flex size-12 shrink-0 items-center justify-center rounded-full bg-secondary-soft text-secondary-foreground">
                ...
                <div className="flex sm:flex-col sm:items-end gap-2 mt-2 sm:mt-0">
                  <Button 
                    variant="outline"
                    nativeButton={false}
          ...
          <RevealItem>
            <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-border bg-card py-16 text-center">
              <div className="flex size-16 items-center justify-center rounded-full bg-muted text-muted-foreground">
                <CalendarBlankIcon className="size-8" />
              </div>
              <p className="mt-4 font-medium">Belum ada booking</p>
              <p className="mt-1 text-sm text-muted-foreground max-w-sm">
                Kamu belum memiliki riwayat booking untuk kategori ini.
              </p>
              {activeTab === "upcoming" && (
                <Button 
                  className="mt-6"
