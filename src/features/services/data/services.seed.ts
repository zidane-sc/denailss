...
/**
 * Seed catalog for services — the initial, non-editable baseline. The LIVE
 * catalog the site renders is managed by the admin services store
 * (`services-admin.mock.ts`), which seeds from this array and persists owner
 * edits to localStorage. Consumers that need the current catalog should read
 * from that store, not this constant. Swap for a real repository later.
 */
