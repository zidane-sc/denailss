...
- **Estimate pricing for nail art** — the Nail Art service carries a `priceNote` ("Estimasi, harga final sesuai desain & tingkat kesulitan — dikonfirmasi via WhatsApp"). When booked without a catalog design, the summary/confirmation label it as an estimate (`±` price, "Estimasi"/"Total estimasi") since the final price depends on the custom request quoted by the artist. Fixed-price services (removal/manicure/pedicure/gel) are unaffected.
